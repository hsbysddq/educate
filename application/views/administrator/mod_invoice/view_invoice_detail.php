                  ...
                  <tbody>
                    ...
                    if ($this->session->level == 'admin'){
                      echo "<tr><th scope='row'>Blokir</th>                   <td>"; if ($rows['blokir']=='Y'){ echo "<input type='radio' name='h' value='Y' checked> Ya &nbsp; <input type='radio' name='h' value='N'> Tidak"; }else{ echo "<input type='radio' name='h' value='Y'> Ya &nbsp; <input type='radio' name='h' value='N' checked> Tidak"; } echo "</td></tr>
							...
							if ($rows['level']=='free'){ 
								echo "<select name='z'><option value='free' selected>free</option><option value='premium'>premium</option></select>"; 
							}else{ 
								echo "<select name='z'><option value='free'>free</option><option value='premium' selected>premium</option></select>"; 
							} 
							echo "</td></tr>
                      ...
                      <tr><th scope='row'>Hak Akses</th>                    <td><div class='checkbox-scroll'>";
                                                                               foreach ($akses as $ro){
                                                                                 echo "<span style='display:block'><a class='text-danger' href='".base_url()."$this->haladmin/delete_akses/$ro[id_umod]/".$this->uri->segment(3)."'><span class='glyphicon glyphicon-remove'></span></a> $ro[nama_modul]</span> ";
                                                                               }
                      echo "</div></td></tr>";
                    }
                  echo "</tbody>
